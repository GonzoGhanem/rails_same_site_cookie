...
module RailsSameSiteCookie
  class UserAgentChecker
    ...
    def is_uc_version_at_least?(major,minor,build)
      if user_agent.version && user_agent.version.major.to_i == major
        if user_agent.version && user_agent.version.minor.to_i == minor
          return user_agent.version && user_agent.version.patch.to_i >= build
        else
          return user_agent.version && user_agent.version.minor.to_i > minor
        end
      else
        return user_agent.version && user_agent.version.major.to_i > major
      end
    end
